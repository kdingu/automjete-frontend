    ...
    return (
      ...
      >
        {field.filters.map((filter, index, arr) => {
          return (
            <div key={`section_${field.facet}_${filter.filter}`} className={arr.length !== index + 1 ? "mb-3" : ""}>
              {form.methods.getComponentForFilter(filter)}
            </div>
          );
        })}
            ...
            return (
              <div key={`wrapper_location_${location.facet}`}>
                {location.filters.map((filter, index, arr) => {
                  return (
                    <div key={`location_filter_${location.facet}_${filter.filter}`} className={arr.length !== index + 1 ? "mb-3" : ""}>
                      {form.methods.getComponentForFilter(filter)}
                    </div>
                  );
                })}
