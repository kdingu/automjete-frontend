        ...
        <div>
                ...
                {mainBrands.map(brand => {
                    return (
                        <div key={brand.slug} className="w-fit m-auto">
                            <SimpleTile href={`/search/brand/${brand.slug}`} imageUrl={brand.logo.externalUrl}
                                        text={brand.name}/>
                        </div>
                    );
                })}
            ...
            {secondaryBrands.length > 0 && (
                <div className="text-center">
                    <Button variant="text"
                            className="border border-transparent px-2 py-1 hover:border-teal-400 !text-black"
            ...
            <div
                className={`mt-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-10 gap-y-5 justify-start hidden ${isOpened ? "!grid" : ""}`}>
                {secondaryBrands.map(brand => {
                    return (
                        <div key={brand.slug} className="w-fit m-auto">
                            <SmallSimpleTile href={`/search/brand/${brand.slug}`} imageUrl={brand.logo.externalUrl}
                                             text={brand.name}/>
                        </div>
                    );
                })}
