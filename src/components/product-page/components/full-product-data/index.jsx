    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap justify-between mb-5">
                {product.kilometers &&
                    <InfoWithIcon iconName="mileageIcon" title={'Mileage'} description={product.kilometers}/>}
                {product.firstRegistration && <InfoWithIcon iconName="registrationIcon" title={'Registration'}
                                                            description={product.firstRegistration}/>}
                {product.previousOwners &&
                    <InfoWithIcon iconName="ownersIcon" title={'Owners'} description={product.previousOwners || 1}/>}
            </div>
                ...
                <Accordion>
                    {attributeGroups.map((attribute) => (
                        <AccordionItem key={attribute} title={t(`${attribute}Label`)}>
                            {groupedAttributes[attribute].map(feature => <div key={feature.attributeName}
                                                                              className="pl-10 pt-4 pb-4">{feature.attribute.label}</div>)}
                        </AccordionItem>
                    ))}
                </Accordion>
            </div>
